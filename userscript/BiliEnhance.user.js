	...
	*/
	...
	function queryCoupon(){
		this.isRun = function(){
			...
			"npcitem.jd.hk", "detail.tmall.hk", "detail.vip.com", "product.suning.com"];
			for(var i=0; i<urls.length;i++){
				if(window.location.host.indexOf(urls[i])!=-1){
					return true;
				}
		...
		this.getPlatform = function(){
			...
			if(host.indexOf("detail.tmall")!=-1){
				...
				platform = "jd";
			}else if(host.indexOf("suning.com")!=-1){
				platform = "suning";
			}else if(host.indexOf("detail.vip.com")!=-1){
				platform = "vpinhui";
			}
			return platform;
		};
		...
		this.getGoodsData = function(platform){
			...
			if(platform=="taobao"){
				...
				goodsId = this.getEndHtmlIdByUrl(href).replace("detail-","");
		...
		this.createCouponHtml = function(platform, goodsId, goodsName){
			...
			if(platform=="vpinhui"){ //唯品会的id需要特殊处理
